...
//Hi _, 
//
//Before we get into top gear with year-end activities and the pastoral transition process, I would like to encourage 
//each of the team leaders to take a few moments to fill out their planning calendars for 2019. See the link below.
//
//If you need help on this, and/or will not be able to complete your calendar, by Friday, Oct 19—Please reply 
// to let me know for forward a message to inform the communications director.
//
//You have shepherded so many successful events, groups, and other creative campaigns for our church this year. 
//Every time we communicate what the life of the church is doing, and people participate, it means our 
//community is formed more and more into the shape and image of the Kingdom of God in Nashville. Let’s follow God 
...
function inviteTeamLeaderstoContributebythefirstFridayOfOctober_() {
  ...
  var teamSheetId = Config.get('PLANNING_CALENDAR_FOR_TEAMS_ID');
  var spreadsheet = SpreadsheetApp.openById(teamSheetId);
...
Dear %s: <br><br>\
It’s that time again! Please complete the instructions for <a href='%s'>Promotion Planning Calendars for Teams</a>, on behalf of \
all your team members, by <strong>%s</strong>.<br><br>\
...
function emailTeamLeadersWhoHaveNotDoneAnything_() {
  ...
  for (var team_leaders_array_index = 0; team_leaders_array_index < team_leaders_array.length; team_leaders_array_index++) {
    ...
    var current_team_sheet_array = current_team_sheet.getRange("B:B").getValues();
...
function remindAllTeamLeadersToContributeByThisFriday_() {
...
function emailNaughtyandNice_() {
  ...
  for (var team_leaders_array_index = 0; team_leaders_array_index < team_leaders_array.length; team_leaders_array_index++) {
    ...
    var teamLeadersContributionDueDate = getFirstWeek_();
    var Cvals = current_team_sheet.getRange("C1:C").getValues();
    var Clast = Cvals.filter(String).length;
    var current_team_sheet_array_Col_C = current_team_sheet.getRange("C4:C").getValues();
    var current_team_sheet_array_Col_E = current_team_sheet.getRange("E4:E").getValues();
    var current_team_sheet_array_Col_J = current_team_sheet.getRange("J4:J").getValues();
...
Please would you either revise these events (highlighted in <span style='background-color: #FFFF00'>yellow</span> on your calendar), email me an update, or <a target='_blank' href='%s'>schedule a promotion planning meeting</a> with me so we can work this out together?\
",                          
                                            current_leader_first_name,
                                            current_leader_sheet_url,
                                            stringArrayBreaks,
...
function getStaffArray_() {
  ...
  var sheet = SpreadsheetApp.openById(staffDataSheetId); 
