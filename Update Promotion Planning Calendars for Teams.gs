...
The "Instructions" and "Team" tabs in the <a href="%s">Promotion Planning Calendars for Teams</a> 
component have been automatically updated for the current calendar year.<br><br>\
...
function updateInstructionsAndPromotionPlanningCalendarsForTeams_v4_() {

  var promotionPlanningCalendarForTeams = SpreadsheetApp.getActive(); 

  var templateId = Config.get('PLANNING_CALENDAR_TEMPLATE_ID');
  var promotionPlanningCalendarsForTeamsTEMPLATE = SpreadsheetApp.openById(templateId);  
  
  var staffDataSheetId = Config.get('STAFF_DATA_GSHEET_ID');
  var staffSheet = SpreadsheetApp.openById(staffDataSheetId);
  
  //verify manual execution is intentional and not run too early
  var title = "Warning";
  var prompt = Utilities.formatString(
    "Executing this script will result in the deletion of all present team sheets.\n\n This will mean the loss of any data \
that may have been contributed manually to these sheets.\n\n Do you wish to proceed?"
  );
  
  if (SpreadsheetApp.getUi().alert(title, prompt, Browser.Buttons.YES_NO) != "YES") {
    return;
  }
  ...
  var rangeDates = promotionPlanningCalendarsForTeamsTEMPLATE.getRange("H4:H100");
  ...
  for (var newTeamSheetIndex = 0; newTeamSheetIndex < teams.length; newTeamSheetIndex++) {
    ...
    // copy formulas and replace placeholders in the formula
    ...
    for (var i in formulas) {
      for (var j in formulas[i]) {
        ...
        formulas[i][j] = templateFormula
          .replace(/TEAM/g, teamName)
          .replace(/UPCOMING_YEAR/g, nextYear)
          .replace(/PDC_URL/g, pdcSheetUrl)
      }
    }
...
} // updateInstructionsAndPromotionPlanningCalendarsForTeams_v4_()
